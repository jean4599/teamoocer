	...
	var ConceptAggregation = _react2.default.createClass({
	  ...
	  handleLevelSubmit: function handleLevelSubmit() {
	    ...
	    if (flag == false) {
	      ...
	      Object.keys(conceptsMap).map(function (key, index) {
	        _this.pushLevel(key, { level: conceptsMap[key]['level'] });
	      });
	    }
	  },
	  pushLevel: function pushLevel(key, obj) {
	    _firebase2.default.database().ref(this.state.courseID + '/_concepts/' + key).push(obj);
	  },
