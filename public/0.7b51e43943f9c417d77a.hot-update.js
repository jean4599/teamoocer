...
/***/ function(module, exports, __webpack_require__) {
	...
	var CursorPanel = _react2.default.createClass({
		...
		componentDidMount: function componentDidMount() {
			this.fire = _firebase2.default.database().ref(this.state.courseID + '/_networks/_cursors/');
		...
		updateCursor: function updateCursor(snapshot) {
			...
			if (result) {
				...
				console.log((0, _utils.toArray)(result));
		...
		render: function render() {
			var cursors = this.state.cursors.map(function (cursor, index) {
				return _react2.default.createElement(Cursor, { key: index, x: cursor.x, y: cursor.y, color: 'red' });
			});
